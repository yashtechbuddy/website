<?php include("includes/header.php");
	...
	if(isset($_POST['submit']) and isset($_GET['add']))
	{
		...
		{	
			$image="MP-".rand(0,99999)."_".$_FILES['image']['name'];
			$tpath1='images/manufacturing_process/'.$image; 			 
			move_uploaded_file($_FILES["image"]["tmp_name"], $tpath1);
		}				
			
		
		/*COUNT LAST INSERTED POSITION NO */
		$qry_order="SELECT * FROM tbl_manufacturing_process order by position_order ASC";
		...
		$data = array( 
 		...
 		$qry = Insert('tbl_manufacturing_process',$data);	

 	   
		$_SESSION['msg']="10"; 
		header( "Location:manage_manufacturing_process.php");
		exit;	
	...
	if(isset($_GET['edit_id']))
	{
			...
			$qry="SELECT * FROM tbl_manufacturing_process where id='".$_GET['edit_id']."'";
	...
	if(isset($_POST['submit']) and isset($_POST['edit_id']))
	{
		...
		if($_FILES['image']['name']!="")
		{
			$img_res=mysqli_query($mysqli,'SELECT * FROM tbl_manufacturing_process WHERE id='.$_GET['edit_id'].'');
			$img_res_row=mysqli_fetch_assoc($img_res);
		
			if($img_res_row['image']!="")
			{
				unlink('images/manufacturing_process/'.$img_res_row['image']);
			}
			...
			$category_edit=Update('tbl_manufacturing_process', $data, "WHERE id = '".$_POST['edit_id']."'");
...
<div class="breadcrumbbar">
   <div class="row align-items-center">
      <div class="col-md-8 col-lg-8">
         <h2 class="page-title"><?php if(isset($_GET['edit_id'])){?>Edit<?php }else{?>Add<?php }?> Manufacturing Process</h2>
      </div>
   </div>
</div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                      <div class="fileupload_block">
                            ...
                            <?php if(isset($_GET['edit_id']) and $row['image']!="") {?><img type="image" src="images/manufacturing_process/<?php echo $row['image'];?>" alt="image" style="width: 172px;height:auto;margin-bottom:10px;"/>
                        	<?php } else {?>
