<?php include("includes/header.php");
	...
	if(isset($_POST['submit']) and isset($_GET['add']))
	{
		...
		{	
			$image="Client-".rand(0,99999)."_".$_FILES['image']['name'];
			$tpath1='images/client/'.$image; 			 
		...
		$qry_order="SELECT * FROM tbl_client order by position_order ASC";
	...
	if(isset($_GET['edit_id']))
	{
			...
			$qry="SELECT * FROM tbl_client where id='".$_GET['edit_id']."'";
	...
	if(isset($_POST['submit']) and isset($_POST['edit_id']))
	{
		...
		if($_FILES['image']['name']!="")
		{
			$img_res=mysqli_query($mysqli,'SELECT * FROM tbl_client WHERE id='.$_GET['edit_id'].'');
			$img_res_row=mysqli_fetch_assoc($img_res);
		
			if($img_res_row['image']!="")
			{
				unlink('images/client/'.$img_res_row['image']);
			}
			...
			$category_edit=Update('tbl_client', $data, "WHERE id = '".$_POST['edit_id']."'");
...
<div class="breadcrumbbar">
   <div class="row align-items-center">
      <div class="col-md-8 col-lg-8">
         <h2 class="page-title"><?php if(isset($_GET['edit_id'])){?>Edit<?php }else{?>Add<?php }?> Client</h2>
      </div>
   </div>
</div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                      <div class="fileupload_block">
                            ...
                            <?php if(isset($_GET['edit_id']) and $row['image']!="") {?><img type="image" src="images/client/<?php echo $row['image'];?>" alt="image" style="width: 172px;height:auto;margin-bottom:10px;"/>
                        	<?php } else {?>
