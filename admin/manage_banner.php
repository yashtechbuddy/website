<?php 
	...
	require ("language/language.php");
     ...
     $qry="SELECT * FROM tbl_banner order by position_order ASC";
	...
	if(isset($_GET['delete_id']))
	{
		...
		$cat_res=mysqli_query($mysqli,'SELECT * FROM tbl_banner WHERE id='.$_GET['delete_id'].'');
		$cat_res_row=mysqli_fetch_assoc($cat_res);


		if($cat_res_row['banner_image']!="")
	    {
	    	unlink('images/banner/'.$cat_res_row['banner_image']);
		}
 
		Delete('tbl_banner','id='.$_GET['delete_id'].'');
...
<div class="breadcrumbbar">
   <div class="row align-items-center">
      <div class="col-md-8 col-lg-8">
         <h2 class="page-title">Manage Banners</h2>
      </div>
	  <div class="col-md-4 col-lg-4">
                        <div class="widgetbar">
							<a href="add_banner.php?add=yes" >
						   <button class="btn btn-primary"><i class="feather icon-plus mr-2"></i>Add Banner</button>	
						   </a>
                        </div>                        
                    </div>
                  ...
                  <td><?php echo $row['banner_name'];?></td>
				   <td>
					<?php if($row['banner_image']!=""){?>
						<img height="50" width="50" style="object-fit:cover;" src="images/banner/<?php echo $row['banner_image'];?>" />
				    <?php }else{ ?><img height="50" width="50" style="object-fit:cover;" src="assets/images/add-image.png" />
					<?php } ?>
				  </td>
				  
                  <td>
				  <a href="add_banner.php?edit_id=<?php echo $row['id'];?>" class="btn btn-dark"><i class="feather ion ion-md-create"></i>  Edit</a>
