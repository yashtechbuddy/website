...
function convertDateFormat($inputDate) {
    ...
    $outputDate = $dateObj->format('d M, Y');
...
function Insert($table, $data){
   ...
   //exit;  
    $qry=mysqli_query($mysqli, "INSERT INTO $table(".implode(",",$fields).") VALUES ('".implode("','", $values )."');") or die( mysqli_error($mysqli) );
...
function Send_GCM_msg($registration_id,$title,$msg,$img,$type)
{
 ...
 $googleApiKey='AIzaSyCvQ-evvjRHWl4avkPme5EMkQ-8jwM523c';
...
function checkSection($id){
	...
	$qry="SELECT * FROM tbl_home_section where visibility_status=0 AND id=".$id."";
